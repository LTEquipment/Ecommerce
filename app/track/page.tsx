    ...
    <div className="wrap content">
      ...
      <div className="track">
        ...
        <form className="track-form no-print" onSubmit={lookup}>
          <div className="field"><label htmlFor="track-id">Order number</label><input id="track-id" value={id} onChange={(e) => setId(e.target.value)} placeholder="e.g. 3f9a1c20 or the full number" required /></div>
          <div className="field"><label htmlFor="track-email">Email on the order</label><input id="track-email" value={email} onChange={(e) => setEmail(e.target.value)} type="email" autoComplete="email" placeholder="you@yourkitchen.com" required /></div>
          <button className="btn btn-primary" type="submit" disabled={busy}><Search /> {busy ? "Looking…" : "Find my order"}</button>
        </form>

        {err && <p className="track-err no-print" role="alert">{err}</p>}
